...
endingwin::draw()
{
    ...
    if (image != NULL) {
        vulture_put_img((vulture_screen->w - image->w) / 2,
                        (vulture_screen->h - image->h) / 2, image);
    ...
    /* Display the rows */
    textpos_y = vulture_screen->h
                - (lines + 1) * vulture_get_lineheight(V_FONT_INTRO);
    for (item_iterator i = items.begin(); i != items.end(); ++i) {
        textpos_x =
            (vulture_screen->w - vulture_text_length(V_FONT_INTRO, i->str))
            / 2;
        vulture_put_text_shadow(V_FONT_INTRO, i->str, vulture_screen,
                                textpos_x, textpos_y, V_COLOR_INTRO_TEXT,
                                V_COLOR_BACKGROUND);
        textpos_y += vulture_get_lineheight(V_FONT_INTRO);
    }

    textpos_x = (vulture_screen->w
                 - vulture_text_length(V_FONT_INTRO, "(press any key)"))
                / 2;
    vulture_put_text_shadow(V_FONT_INTRO, "(press any key)", vulture_screen,
                            textpos_x, textpos_y, V_COLOR_INTRO_TEXT,
                            V_COLOR_BACKGROUND);
...
eventresult
endingwin::handle_mousebuttonup_event(window *target, void *result,
...
eventresult
endingwin::handle_keydown_event(window *target, void *result, int sym,
                                int mod, int unicode)
{
    return V_EVENT_HANDLED_FINAL;
}
