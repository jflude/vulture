...
introwin::introwin(window *p, std::vector<std::string> &imagenames,
...
introwin::draw()
{
    ...
    if (image) {
        ...
        for (j = 0; j < (int) subtitles[current_scene].size(); j++) {
            pos_x = (w
                     - vulture_text_length(V_FONT_INTRO,
                                           subtitles[current_scene][j]))
                    / 2;
            pos_y = 2 * img_y + image->h + lineheight * j;
            vulture_put_text(V_FONT_INTRO, subtitles[current_scene][j],
                             vulture_screen, pos_x, pos_y,
                             V_COLOR_INTRO_TEXT);
...
eventresult
introwin::handle_timer_event(window *target, void *result, int time)
{
    /* next scene? */
...
eventresult
introwin::handle_mousemotion_event(window *target, void *result, int xrel,
                                   int yrel, int state)
{
    /* hide cursor */
...
eventresult
introwin::handle_mousebuttonup_event(window *target, void *result,
...
eventresult
introwin::handle_keydown_event(window *target, void *result, int sym, int mod,
                               int unicode)
{
    /* cancel intro */
    return V_EVENT_HANDLED_FINAL;
...
eventresult
introwin::handle_resize_event(window *target, void *result, int res_w,
                              int res_h)
{
    w = res_w;
    h = res_h;
...
introwin::next_scene()
{
    ...
    if (current_scene > 0
        && imagenames[current_scene - 1] != imagenames[current_scene])
        image_changed = true;
