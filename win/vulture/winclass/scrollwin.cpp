...
scrollwin::draw()
{
    ...
    for (window *winelem = first_child; winelem;
...
int
scrollwin::get_scrollheight(void)
{
    int max_y = 0;
    int lower_edge = 0;
    ...
    /* add one line of extra space, to make sure the last element can be
     * scrolled into view */
...
int
scrollwin::get_menuitem_width(window *item, int colwidths[8])
{
...
scrollwin::layout(void)
{
    ...
    for (winelem = first_child; winelem; winelem = winelem->sib_next) {
        ...
        if (winelem->v_type == V_WINTYPE_OPTION
            ...
            winelem->y = height;
            height += winelem->h + 4;
            /* this is just a guess at this point, as winelem->w may be wrong
             * for multi-column */
            elem_maxwidth =
                (elem_maxwidth > winelem->w) ? elem_maxwidth : winelem->w;
        }
    }

    /* add up the widths of all the columns and determine the starting
...
void
scrollwin::set_height(int scrollheight)
{
...
eventresult
scrollwin::scrollto(int scrolltype, int scrolldir)
{
    ...
    case V_SCROLL_PIXEL_ABS:
        ...
        while (child->sib_next && child->y < scroll_top && child != scroll
               && (child->y < height - this->h))
            child = child->sib_next;
...
eventresult
scrollwin::mousescroll(scrollbar *target, int is_drag)
{
    int scrollind_y;
    point mouse;
    int scrollarea_top = target->abs_y + vulture_winelem.scrollbutton_up->h;
    int scrollarea_bottom =
        target->abs_y + target->h - vulture_winelem.scrollbutton_down->h;

    scrollind_y = ((scrollarea_bottom - scrollarea_top
                    - vulture_winelem.scroll_indicator->h)
                   * (int) target->scrollpos)
                  / 8192.0;
    ...
    else if (mouse.y >= scrollarea_top + vulture_winelem.scroll_indicator->h
                            + scrollind_y) {
        if (!is_drag)
            return scrollto(V_SCROLL_PAGE_REL, 1);
    }
...
eventresult
scrollwin::handle_mousemotion_event(window *target, void *result, int xrel,
                                    int yrel, int state)
{
    window *winelem;
    point mouse, oldpos;
...
eventresult
scrollwin::handle_mousebuttonup_event(window *target, void *result,
