...
void mapdata::set_glyph(int x, int y, int glyph)
{
	...
	if (iflags.window_inited == TRUE && !cansee(x,y) && x == u.ux && y == u.uy) {
	...
	if (iflags.window_inited == TRUE && cansee(x,y))
	{
		...
		* at the start of the turn and dealing with them manually is ugly */
		...
		if ((obj = vobj_at(x,y)) && !covers_objects(x,y)) {
			...
			* corpses... */
			map_obj = vulture_object_to_tile(
				glyph_to_obj(obj_to_glyph(obj, rn2_on_display_rng)), x, y, NULL
			);
		}
		...
		if ((trap = t_at(x,y)) && !covers_traps(x,y) && trap->tseen)
			...
			map_trap = vulture_tilemap_misc[what_trap(trap->ttyp, rn2_on_display_rng) + S_arrow_trap - 1];
		else
