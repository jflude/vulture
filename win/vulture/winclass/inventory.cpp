...
inventory::draw()
{
    ...
    vulture_fill_rect(inner_x_position, inner_y_position,
                      inner_x_position + inner_width - 1,
                      inner_y_position + headline_height * 2,
                      CLR32_BLACK_A50);

    vulture_line(inner_x_position, inner_y_position,
                 inner_x_position + inner_width - 1, inner_y_position,
                 CLR32_GRAY20);
    vulture_line(inner_x_position, inner_y_position + headline_height * 2,
                 inner_x_position + inner_width - 1,
                 inner_y_position + headline_height * 2, CLR32_GRAY20);

    vulture_put_text_shadow(V_FONT_HEADLINE, caption, vulture_screen,
                            inner_x_position
                                + (inner_width - headline_width) / 2,
    ...
    if (ow_ncols > ow_vcols) {
        if (ow_firstcol + 1 == ow_firstcol + ow_vcols)
            ...
            snprintf(label, 32, "Pages %d to %d of %d", ow_firstcol + 1,
                     ow_firstcol + ow_vcols, ow_ncols);

        labelwidth = vulture_text_length(V_FONT_MENU, label);

        vulture_put_text_shadow(V_FONT_MENU, label, vulture_screen,
                                x + (w - labelwidth) / 2, y + h - 21,
                                CLR32_BLACK, CLR32_GRAY20);
    }
...
void
inventory::update_invscroll(int newpos)
{
    window *winelem;
    int itemcount = 0;
    int itemcol;
    ...
    for (winelem = first_child; winelem; winelem = winelem->sib_next) {
        if (winelem->v_type == V_WINTYPE_OBJITEM
            ...
            winelem->x =
                (itemcol - newpos) * (V_LISTITEM_WIDTH + 4) + leftoffset;
            winelem->y =
                (itemcount % ow_vrows) * V_LISTITEM_HEIGHT + topoffset;

            winelem->visible =
                (itemcol >= newpos && itemcol < newpos + ow_vcols);
...
eventresult
inventory::context_menu(objitemwin *target)
{
    int action = 0, key = 0;
    contextmenu *menu;
    ...
    if (!target->obj->owornmask && target->obj->oclass != WAND_CLASS) {
        if (target->obj->oclass != RING_CLASS
            && target->obj->oclass != AMULET_CLASS)
    ...
    if (action) {
        ...
        switch (action) {
        case V_INVACTION_APPLY:
            key = 'a';
            break;
        case V_INVACTION_DRINK:
            key = 'q';
            break;
        case V_INVACTION_EAT:
            key = 'e';
            break;
        case V_INVACTION_READ:
            key = 'r';
            break;
        case V_INVACTION_ZAP:
            key = 'z';
            break;
        case V_INVACTION_WEAR:
            key = 'W';
            break;
        case V_INVACTION_PUT_ON:
            key = 'P';
            break;
        case V_INVACTION_WIELD:
            key = 'w';
            break;
        case V_INVACTION_QUIVER:
            key = 'Q';
            break;
        case V_INVACTION_REMOVE:
             ...
             * amulets of ESP) get recalculated. However we do not want to
             * perform any more actions or cause messages to be printed.
             * CTRL+r (redraw) is a suitable NOP */
            key = CTRL('r');
#endif
...
eventresult
inventory::handle_mousemotion_event(window *target, void *result, int xrel,
                                    int yrel, int state)
{
...
eventresult
inventory::handle_mousebuttonup_event(window *target, void *result,
                                      ...
                                      int state)
{
    ...
    if (select_how == PICK_NONE) {
        ...
        if (this == target
            && (mouse.x < abs_x || mouse.y < abs_y || mouse.x > abs_x + w
                || mouse.y > abs_y + h))
            return V_EVENT_HANDLED_FINAL;

        /* left clicks on object items do nothing */
        if (button == SDL_BUTTON_LEFT && target != this
            && target->v_type == V_WINTYPE_OBJITEM)
            return V_EVENT_HANDLED_NOREDRAW;
    ...
    if (button == SDL_BUTTON_WHEELUP) {
        ...
        /* select a range of items from target (clicked item) to
         * handler->pd.ow_lasttoggled (previously clicked item) */
        if (target->v_type == V_WINTYPE_OBJITEM
            && (SDL_GetModState() & KMOD_LSHIFT) && select_how != PICK_ONE
            && ow_lasttoggled) {
            int selectme = 0;
            for (winelem = first_child; winelem;
...
eventresult
inventory::handle_keydown_event(window *target, void *result, int sym,
                                int mod, int unicode)
{
    window *winelem;
    ...
    case SDLK_ESCAPE:
        *(int *) result =
            (select_how == PICK_NONE) ? V_MENU_ACCEPT : V_MENU_CANCEL;
        return V_EVENT_HANDLED_FINAL;
    ...
    case MENU_SEARCH:
        ...
        str_to_find[0] = '\0';
        if (vulture_get_input(-1, -1, "What are you looking for?",
                              str_to_find)
            != -1) {
            itemcount = 0;
            for (winelem = first_child; winelem;
    ...
    default:
        ...
        if (key >= '0' && key <= '9' && ow_lasttoggled
            ...
            ow_lasttoggled->item->count =
                ow_lasttoggled->item->count * 10 + (key - '0');
        ...
        for (std::vector<window *>::iterator target = targets_found.begin();
             target != targets_found.end(); ++target) {
            select_option(static_cast<objitemwin *>(*target), -1);
            if (select_how == PICK_ONE) {
                *(int *) result = V_MENU_ACCEPT;
                return V_EVENT_HANDLED_FINAL;
...
eventresult
inventory::handle_resize_event(window *target, void *result, int res_w,
                               int res_h)
{
    if (visible) {
...
inventory::layout()
{
    ...
    int bottomoffset = 60; /* guesstimate for bottom border + page arrows +
                              minimal spacing */
    ...
    maxitems_per_col =
        (parent->h - topoffset - bottomoffset) / V_LISTITEM_HEIGHT;

    maxitems_per_row =
        (parent->w - 2 * leftoffset - 10) / (V_LISTITEM_WIDTH + 4);

    /* calc number of rows to contain itemcount items */
    ncols = itemcount / maxitems_per_col
            + (itemcount % maxitems_per_col != 0 ? 1 : 0);
    ...
    for (winelem = first_child; winelem; winelem = winelem->sib_next) {
        if (winelem->v_type == V_WINTYPE_OBJITEM
            || winelem->v_type == V_WINTYPE_OBJITEMHEADER) {
            winelem->x =
                (itemcount / ow_vrows) * (V_LISTITEM_WIDTH + 4) + leftoffset;
            winelem->y =
                (itemcount % ow_vrows) * V_LISTITEM_HEIGHT + topoffset;
            ...
            if (winelem->v_type == V_WINTYPE_OBJITEM) {
                /* nethack may have been nice and passed an object pointer in
                 * menu_id_v Unforunately, we need this ugly hack to try to
                 * discern between chars, small ints and pointers */
                if (winelem->menu_id_v > (void *) 0x10000)
                    static_cast<objitemwin *>(winelem)->obj =
                        (struct obj *) winelem->menu_id_v;
    ...
    if (ncols > ow_vcols) {
        ...
        btn->w = 100;
        btn->h = 24;
        btn->image = vulture_get_img_src(
            0, 0, vulture_winelem.invarrow_left->w,
        ...
        btn->h = 24;
        btn->image =
            vulture_get_img_src(0, 0, vulture_winelem.invarrow_right->w,
                                vulture_winelem.invarrow_right->h,
                                vulture_winelem.invarrow_right);
        btn->visible = 1;
    }
    ...
    if (select_how != PICK_NONE) {
        ...
        if (select_how == PICK_ANY) {
            ...
            btn->w = max_width;
            btn->x =
                (w - rightoffset - leftoffset - total_width) / 2 + leftoffset;
        }
        ...
        btn->image = vulture_get_img_src(0, 0, vulture_winelem.closebutton->w,
                                         vulture_winelem.closebutton->h,
                                         vulture_winelem.closebutton);
    }
