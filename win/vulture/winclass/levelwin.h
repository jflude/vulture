...
class levelwin : public window, public mapviewer
{
  public:
    ...
    void map_update(glyph_type type, int prev_glyph, int new_glyph, int x,
                    int y);
    void map_clear();
  ...
  private:
    ...
    int get_wall_decor(int floortype, int wally, int wallx, int floory,
                       int floorx);
    ...
    unsigned char map_deco[ROWNO]
                          [COLNO]; /* positions of murals and carpets */
    point map_highlight;

    /* pointer to full height, half height or transparent walltile array */
    struct walls *walltiles;

    struct walls maptile_wall[ROWNO][COLNO]; /* Custom (combination) wall
                                                style for each square */
    struct fedges maptile_floor_edge[ROWNO][COLNO]; /* Custom floor edge style
                                                       for each square */
