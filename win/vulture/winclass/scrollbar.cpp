...
scrollbar::draw()
{
    ...
    vulture_put_img(abs_x, scrollarea_bottom,
                    vulture_winelem.scrollbutton_down);

    /* draw the scrollbar backgound */
    vulture_set_draw_region(abs_x, scrollarea_top,
                            abs_x + vulture_winelem.scrollbar->w - 1,
                            scrollarea_bottom - 1);
    for (pos_y = scrollarea_top; pos_y < scrollarea_bottom;
         pos_y += vulture_winelem.scrollbar->h)
     ...
     * failing to consider spacing between menu elements will lead to
     * scrollpos values that are substantially larger than 8192 (100%). That's
     * OK for the main menu area (desired even), but not for the scroll
     * indicator */
    scroll_pos = (scrollpos <= 8192) ? scrollpos : 8192;

    pos_y = scrollarea_top
