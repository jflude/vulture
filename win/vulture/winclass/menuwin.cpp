...
menuwin::draw()
{
    ...
    /* no need to invalidate the draw region, the call to draw mainwin did
     * that for us */
    return true;
}

void
menuwin::select_option(optionwin *target, int count)
{
    window *winelem;
    optionwin *opt;
...
eventresult
menuwin::handle_mousemotion_event(window *target, void *result, int xrel,
                                  int yrel, int state)
{
...
eventresult
menuwin::handle_mousebuttonup_event(window *target, void *result, int mouse_x,
...
eventresult
menuwin::handle_keydown_event(window *target, void *result, int sym, int mod,
                              int unicode)
{
    optionwin *opt;
    window *winelem;
    ...
    case MENU_UNSELECT_ALL:
        ...
        for (winelem = scrollarea->first_child; winelem;
    ...
    default:
        ...
        for (std::vector<window *>::iterator target = targets_found.begin();
            ...
            select_option(static_cast<optionwin *>(*target),
                          count ? count : -1);
            count = 0;
            if (select_how == PICK_ONE) {
                *(int *) result = V_MENU_ACCEPT;
...
eventresult
menuwin::handle_resize_event(window *target, void *result, int w, int h)
{
    layout();
    return V_EVENT_HANDLED_NOREDRAW;
}
...
void
menuwin::assign_accelerators()
{
    int new_accel;
    char used_accels[128]; /* 65 should be max # of accels, the other 63 bytes
                              are safety :P */
...
menuwin::layout()
{
    ...
    menu_height_limit =
        parent->get_h() - get_frameheight() - buttonheight - 30;
...
/* selection iterator */
menuwin::selection_iterator::selection_iterator(item_iterator start,
                                                item_iterator end)
    : iter(start), end(end)
{
...
menuwin::selection_iterator &
menuwin::selection_iterator::operator++(void)
{
    iter++;
    while (iter != end && !iter->selected)
        iter++;
...
bool
menuwin::selection_iterator::operator!=(selection_iterator rhs) const
{
